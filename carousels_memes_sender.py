...
async def grasp_rows(db_pool, hours):
...
async def carousels_sender(data:telegram.InputMediaPhoto, chat_id) -> None:
    temp_list = []
    for k,v in data.items():
        temp_list.append(v)
    await asyncio.sleep(0.3)
    await bot.send_media_group(media = await carousels_converter(temp_list), chat_id = chat_id)
    await asyncio.sleep(0.3)
...
async def job(connection, chat_id, hours):
    ...
    for x in an_array:
        for y in list(x):
            temp_y = json.loads(y)
            try:
                await carousels_sender(temp_y, chat_id)
            except Exception as _ex:
                print(f'!Errore in file {__file__} :  THE ERROR is :',_ex)


async def main_carousel_sender(chat_id, hours = 6):

    """Main function"""
    try:
        connection = await asyncpg.connect(host = db_host, user = db_user, password = db_password
        , database = db_db_name)
        await asyncio.create_task(job(connection, chat_id, hours))
    except Exception as _ex:
        print('Error:', _ex)
    finally:
        if connection:
...
#     asyncio.run(main_carousel_sender(chat_id = 189382736))